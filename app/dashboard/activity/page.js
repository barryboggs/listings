    ...
    <>
      <div className="flex flex-wrap justify-between items-center gap-3 mb-5">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "#666" }}>All API interactions logged with user attribution</p>
        </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "#151517", border: "1px solid #1e1e22" }}>
        {filtered.map((entry, i) => {
          ...
          return (
            <div key={entry.id} className="flex items-start gap-4 px-5 py-3.5 transition-colors" style={{ background: i % 2 === 0 ? "#151517" : "#131315", borderBottom: i < filtered.length - 1 ? "1px solid #1a1a1d" : "none" }}>
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center text-sm" style={{ color: "#555" }}>No activity matches your filters</div>
        )}
      </div>

      <div className="mt-5 p-4 rounded-lg" style={{ background: "#1a1a1d", border: "1px solid #222" }}>
        <p className="text-xs leading-relaxed" style={{ color: "#777" }}>
          Activity is logged automatically when locations are updated through this app. New entries appear at the top. For production, persist to a database for permanent retention and CSV/PDF export.
        </p>
      </div>
    </>
  );
