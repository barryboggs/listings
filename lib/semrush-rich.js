 ...
 *
 * Termination is on EMPTY page, not short page — the new API appears to
 * cap responses at 50 items regardless of requested page_size, so a "short"
 * page is not a reliable end-of-data signal. Default page_size 50 aligns
 * with the apparent cap so we don't pay for over-requesting.
 *
 * Safety cap at 500 pages prevents runaway loops if pagination is broken.
 */
export async function getAllRichLocations({ page_size = 50 } = {}) {
  const all = [];
  ...
  while (page <= MAX_PAGES) {
    ...
    if (items.length === 0) break; // empty = past end

    // Detect "API ignores page param and keeps returning the same data"
    // by checking whether the first ID of this page matches page 1's first ID.
    if (page === 1) {
      firstIdOfPage1 = items[0]?.location_id;
    } else if (firstIdOfPage1 && items[0]?.location_id === firstIdOfPage1) {
      console.warn(
        `[semrush-rich] page ${page} returned the same first item as page 1 — pagination may not be honored. Stopping at ${all.length} items.`
      );
      break;
    }
    ...
    for (const item of items) {
      if (item.location_id && seenIds.has(item.location_id)) continue;
      if (item.location_id) seenIds.add(item.location_id);
      all.push(item);
      newOnThisPage++;
    }

    // If a page returns only duplicates, we've hit the end (or the API is
    // looping). Either way: stop.
  ...
  if (page > MAX_PAGES) {
    console.warn(
      `[semrush-rich] getAllRichLocations hit MAX_PAGES (${MAX_PAGES}) — returned ${all.length} items but more may exist`
    );
  }
