...
/**
 * Upload an image to a location by reference (sourceUrl).
 *
 * Body shape (confirmed empirically — the image endpoint uses camelCase
 * because the underlying handler is a protobuf service in strict JSON
 * mode, unlike the rest of the rich API which is snake_case):
 *   { sourceUrl, category }
 *
 * Reasonable categories to try: LOGO, COVER, ADDITIONAL, INTERIOR, EXTERIOR.
 * The exact accepted set is still TBD via the probe.
 *
 * The validate-only query param is named `validate_only` consistent with
 * Semrush's location endpoints — that part is NOT proto-style. Semrush is
 * inconsistent between path/query (snake) and body (camel) for image ops.
 */
export async function createLocationImage(locationId, { sourceUrl, category = "ADDITIONAL", validateOnly = false } = {}) {
  if (!locationId) throw new Error("locationId is required");
