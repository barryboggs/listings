...
export async function initDatabase() {
  ...
  try {
    ...
    `;
    // Authoritative per-shop coordinates from the internal Driven Brands
    // shop database. Admin imports via CSV on /dashboard/map-markers.
    // Used to fix Semrush's map pin when GBP has no latlng either (~10%
    // of GBP locations lack lat/lng — unverified, service-area, etc.).
    // Stored as DOUBLE PRECISION to match Google/Semrush's format.
    await sql`
      ALTER TABLE lm_shop_numbers ADD COLUMN IF NOT EXISTS latitude DOUBLE PRECISION
    `;
    await sql`
      ALTER TABLE lm_shop_numbers ADD COLUMN IF NOT EXISTS longitude DOUBLE PRECISION
    `;
    await sql`
      ALTER TABLE lm_shop_numbers ADD COLUMN IF NOT EXISTS coordinates_updated_at TIMESTAMPTZ
    `;
...
/**
 * Bulk-set per-shop lat/lng from CSV import. Admins upload a CSV of
 * (shop_id, latitude, longitude) rows sourced from the internal Driven
 * Brands shop database — used to fix Semrush map pins when GBP has no
 * latlng to source from.
 *
 * Silently skips rows where the shop_id isn't in lm_shop_numbers (the
 * CSV might include shops we haven't imported yet). Returns per-row
 * counts so the UI can report matched vs unmatched.
 *
 * @param {Array<{shopId, latitude, longitude}>} rows
 */
export async function bulkSetShopCoordinates(rows) {
  if (!Array.isArray(rows) || rows.length === 0) {
    return { updated: 0, unmatched: 0, errors: [] };
  }
  ...
  if (!hasPostgres()) {
    ...
    const now = new Date().toISOString();
    for (const r of rows) {
      const shop = memShopNumbers.find((s) => s.shop_id === r.shopId);
      if (shop) {
        shop.latitude = r.latitude;
        shop.longitude = r.longitude;
        shop.coordinates_updated_at = now;
        updated++;
      } else unmatched++;
    }
  ...
  for (const r of rows) {
    try {
      ...
      if (errors.length < 5) errors.push(`${r.shopId}: ${e.message}`);
    }
  }
 ...
 * `brand` (optional) scopes the query — useful when the admin wants
 * to check "is my AGN offer queued for cleanup" without wading through
 * every other brand's expired rows.
 */
export async function findExpiredOfferPushes({ beforeDate = null, brand = null, limit = 500 } = {}) {
...
/**
 * Pre-load already-known reviews for a set of shops. Returns a Map keyed
 * by review_name with values `{ google_updated_at }`. Used by the sync
 * route's incremental optimization: while paginating a shop's reviews
 * (ordered by updateTime desc), we can stop as soon as we hit a review
 * we already have with the same updateTime — everything past that
 * point is guaranteed unchanged and re-fetching wastes API + DB calls.
 *
 * Passed an array of gbp_location_ids; batches into one query.
 */
export async function getKnownReviewsForShops(gbpLocationIds) {
  if (!Array.isArray(gbpLocationIds) || gbpLocationIds.length === 0) {
    return new Map();
  }

  if (!hasPostgres()) {
    const set = new Set(gbpLocationIds);
    const out = new Map();
    for (const r of memReviews) {
      if (set.has(r.gbp_location_id)) {
        out.set(r.review_name, { google_updated_at: r.google_updated_at });
      }
    }
    return out;
  }
  ...
  try {
    const sql = await db();
    // ANY($1::text[]) is Vercel Postgres's way of matching a value
    // against an array — much cheaper than N per-shop queries when a
    // chunk covers 30 shops.
    const { rows } = await sql`
      SELECT review_name, google_updated_at
      FROM lm_reviews
      WHERE gbp_location_id = ANY(${gbpLocationIds}::text[])
    `;
    const out = new Map();
    for (const r of rows) {
      out.set(r.review_name, { google_updated_at: r.google_updated_at });
    ...
    console.error("getKnownReviewsForShops error:", e.message);
    return new Map();
  }
}
...
/**
 * Count reviews still needing enrichment, matching the same filter shape
 * as getUnenrichedReviews. Cheap (index-only COUNT). Used by the
 * enrichment route so the response can return an exact `remaining`
 * count instead of the "1+" hint we had originally — enables an
 * accurate progress bar on the client.
 */
export async function countUnenrichedReviews({ brand = null, monthStr = null } = {}) {
  ...
  try {
    ...
    console.error("countUnenrichedReviews error:", e.message);
    return 0;
  }
}
