...
/**
 * GET a single GBP location by its resource name (locations/{id}). Used
 * by the map-marker repush flow to fetch the current stored `latlng`
 * before re-patching it (the theory: touching the field triggers
 * Google to re-process/re-verify without changing the coordinates).
 *
 * `readMask` scopes the response to just what we need — GBP returns a
 * lot of fields by default which we don't use here.
 */
export async function getGbpLocation(gbpLocationId, { readMask = "name,title,latlng,storefrontAddress" } = {}) {
  if (!gbpLocationId) throw new Error("gbpLocationId is required");
  const params = new URLSearchParams({ readMask });
  const path = `${BUSINESS_INFO_BASE}/${withLocationsPrefix(gbpLocationId)}?${params.toString()}`;
  return googleBpFetch(path);
}

/**
 * PATCH a GBP location's `latlng` field. Used by the map-marker
 * bulk-repush flow. Google's updateMask on this endpoint accepts
 * `latlng` as a single field name; the body is the partial location
 * with the field to update.
 *
 * The typical use here is to re-push the SAME coordinates Google
 * currently has stored — the theory being that Google re-processes
 * the location and clears the "pin needs adjustment" flag Semrush
 * has been surfacing. If it doesn't work, admin still has to fix
 * pins manually in the GBP dashboard.
 *
 * Note: Google's response to programmatic latlng changes varies —
 * sometimes auto-applied, sometimes queued for review. We report
 * whatever comes back per-shop.
 */
export async function updateGbpLocationLatLng(gbpLocationId, latlng) {
  if (!gbpLocationId) throw new Error("gbpLocationId is required");
  if (!latlng || typeof latlng.latitude !== "number" || typeof latlng.longitude !== "number") {
    throw new Error("latlng must be an object with numeric latitude and longitude");
  }
  const params = new URLSearchParams({ updateMask: "latlng" });
  const path = `${BUSINESS_INFO_BASE}/${withLocationsPrefix(gbpLocationId)}?${params.toString()}`;
  return googleBpFetch(path, {
    method: "PATCH",
