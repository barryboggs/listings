export const BRANDS = [
  { id: "carstar", name: "CARSTAR", color: "#E31837" },
  { id: "take5", name: "Take 5 Oil Change", color: "#0066CC" },
  { id: "autoglass", name: "Auto Glass Now", color: "#00875A" },
  { id: "abra", name: "ABRA Auto Body", color: "#8B5CF6" },
  { id: "fixauto", name: "Fix Auto USA", color: "#F97316" },
  { id: "maaco", name: "Maaco", color: "#DC2626" },
  { id: "meineke", name: "Meineke", color: "#EAB308" },
  { id: "econo", name: "Econo Lube", color: "#14B8A6" },
  { id: "1800radiator", name: "1-800-Radiator & A/C", color: "#6366F1" },
  { id: "starlube", name: "Star Lube", color: "#EC4899" },
];

// Fallback colors for brands not in the list above
const FALLBACK_COLORS = [
  "#06B6D4", "#8B5CF6", "#F43F5E", "#84CC16", "#F59E0B",
  "#3B82F6", "#10B981", "#EF4444", "#A855F7", "#22D3EE",
];

/**
 * Get brand config by id. Returns a default if the brand isn't in the known list.
 */
export function getBrandConfig(brandId) {
  const known = BRANDS.find((b) => b.id === brandId);
  if (known) return known;

  // Generate a stable color from the brand id
  let hash = 0;
